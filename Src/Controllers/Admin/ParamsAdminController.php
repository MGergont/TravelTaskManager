...
use Src\Models\Admin\ParamsAdminModel;

class ParamsAdminController extends AbstractController
{
    public function ParamsAdminView(): Void
    {
        $this->adminDashboard();

        if (isset($_SESSION['status']) && $_SESSION['status'] === "login") {
            $dashboardAdminModel = new ParamsAdminModel($this->configuration);

            if($result = $dashboardAdminModel->getSettingsAdmin($_SESSION['userId'])){
                $this->paramView = $result;
            }

            (new View())->renderAdmin("paramAdmin", $this->paramView, "admin");
        } else {
    ...
    public function PwdUnlock(): void
    {
        ...
        if (!empty($data['pwdUnlock']) && $data['pwdUnlock'] === "on") {
            if ($dashboardAdminMod->accountUnloc("active", (int) $data['id'])) {
                flash("adminManagement", "Konto zostało odblokowane", "alert alert--confirm");
            } else {
                flash("adminManagement", "Coś poszło nie tak", "alert alert--error");
        ...
        if (!empty($data['pwdChange']) && $data['pwdChange'] === "on") {
            ...
            if ($this->ValidPwd($data['pwd'], $data['pwdRepeat'])) {
                flash("adminManagement", "Niepoprawne hasła", "alert alert--error");
                $this->redirect("/admins");
            } else {
                $data['pwd'] = password_hash($data['pwd'], PASSWORD_DEFAULT);

                if ($dashboardAdminMod->pwdChange($data['pwd'], (int) $data['id'])) {
                    flash("adminManagement", "Hasło zostało zmienione", "alert alert--confirm");
                } else {
                    flash("adminManagement", "Coś poszło nie tak", "alert alert--error");
                    $this->redirect("/admins");
                }
        ...
        if (empty($data['pwdUnlock']) || empty($data['pwdChange']) || empty($data['pwd']) || empty($data['pwdRepeat'])) {
            flash("adminManagement", "Nie uzupełniono odpowiednich formularzy", "alert alert--error");
            $this->redirect("/admins");
        }
    ...
    public function accountDell(): void{
        ...
        if ($dashboardAdminMod->accountDell((int) $data['id'])) {
            flash("adminManagement", "Konto zostało usunięte", "alert alert--confirm");
            $this->redirect("/admins");
        } else {
            flash("adminManagement", "Coś poszło nie tak", "alert alert--error");
            $this->redirect("/admins");
    ...
    public function accountEdit(): void{
        ...
        if($this->IfMaxLength($data, 30)){
            flash("adminManagement", "Nieprawidłowa długość znaków", "alert alert--error");           
            $this->redirect("/admins");
        };
        //TODO bez znaków specjalnych i polskich
        if($this->IfSpecialAndPolishCharacters($data['login'])){
            flash("adminManagement", "Niepoprawne znaki w nazwie", "alert alert--error");           
            $this->redirect("/admins");
        }
        //TODO bez znaków specjalnych
        if($this->IfSpecialCharacters($data['name'])){
            flash("adminManagement", "Niepoprawne znaki w danych wprowadzonych w formularzu", "alert alert--error");           
            $this->redirect("/admins");
        }
        ...
        if($this->ValidPhoneNumber($data['phoneNumber'])){
            flash("adminManagement", "Niepoprawny numer telefonu", "alert alert--error");           
            $this->redirect("/admins");
        }
        //TODO walidacj adresu email
        if($this->ValidEmail($data['email'])){
            flash("adminManagement", "Niepoprawny adres email", "alert alert--error");           
            $this->redirect("/admins");
        }

        if($this->ValidStatus($data['status'])){
            flash("adminManagement", "Nieprawidłowe wartości w statusie konta", "alert alert--error");           
            $this->redirect("/admins");
        }
    
        if ($dashboardAdminMod->accountUpdate($data)) {
            flash("adminManagement", "Konto zostało zmodyfikowane", "alert alert--confirm");
            $this->redirect("/admins");
        } else {
            flash("adminManagement", "Coś poszło nie tak", "alert alert--error");
            $this->redirect("/admins");
